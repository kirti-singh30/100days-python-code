...
with open("day24_Mail Merge Project Start/Input/Letters/starting_letter.txt") as letter_file:
    letter_content = letter_file.read()
    #print(letter_content)

with open("day24_Mail Merge Project Start/Input/Names/invited_names.txt") as names_file:
    names = names_file.readlines()
    for name in names:
        strip_name = name.strip()
        new_letter = letter_content.replace(PLACEHOLDER,strip_name)
        with open(f"day24_Mail Merge Project Start/Output/ReadyToSend {strip_name}.txt","w") as complete_letter:
